...
@Component({
  selector: 'app-ecom004-report',
  templateUrl: './ecom004-report.component.html',
  styleUrls: ['./ecom004-report.component.scss']
})
export class Ecom004ReportComponent implements OnInit, OnDestroy {
  loading = false
  isDialogOpen1 = false
  functionID = 0
  //object
  ...
  getLocalStorageModule() {
    ...
    if (Ps_UtilObjectService.hasValueString(moduleApiCache)) {
      ...
      // functionApi = moduleApi.ListFunctions.find(s => s.DLLPackage.toLowerCase().includes('ecom004-report'))
      var findFunctionApi = moduleApi.ListGroup.find(s => s.ModuleID == 'ecom-report')
      functionApi = findFunctionApi.ListFunctions.find(s => s.DLLPackage.toLowerCase() == 'ecom004-report')
