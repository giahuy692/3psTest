...
@Component({
  selector: 'app-ecom007-inport-list',
  templateUrl: './ecom007-inport-list.component.html',
  styleUrls: ['./ecom007-inport-list.component.scss']
})
export class Ecom007InportListComponent implements OnInit, OnDestroy {
  loading = false
  isAdd = true
  isFilterActive = true
  excelValid = true
  ...
  filterToWarehouse: FilterDescriptor = {
    field: "ToWarehouse", operator: "eq", value: this.mainWarehouse
  }
  //filder date
  ...
  filterFromWarehouseName: FilterDescriptor = {
    field: "FromWarehouseName", operator: "contains", value: this.mainWarehouse
  }
  ...
  loadFilter() {
    ...
    this.filterToWarehouse.value = this.mainWarehouse
    this.gridState.filter.filters = [this.filterToWarehouse]
    ...
    //search box
    if (Ps_UtilObjectService.hasValueString(this.filterFromWarehouseName.value))
      this.filterSearchBox.filters.push(this.filterFromWarehouseName)
  ...
  getActionDropdown(moreActionDropdown: MenuDataItem[], dataItem: DTOInport) {
    ...
    if (this.isToanQuyen || this.isAllowedToVerify) {
      if (statusID == 1) {
        moreActionDropdown.push({ Name: "Nhận hàng", Code: "redo", Type: 'StatusID', Link: "1", Actived: true })
      }
      else if (statusID == 2) {
        moreActionDropdown.push({ Name: "Hoàn tất nhận hàng", Code: "check-outline", Type: 'StatusID', Link: "2", Actived: true })
      }
      else if (statusID == 3) {
  ...
  getSelectionPopup(selectedList: DTOInport[]) {
    ...
    if (canNhanHang_canXoa != -1 && (this.isToanQuyen || this.isAllowedToCreate))
      moreActionDropdown.push({
        Type: "StatusID", Name: "Nhận hàng", Code: "redo", Link: "1", Actived: true, LstChild: []
      })
    //
    if (this.isToanQuyen || this.isAllowedToVerify) {
      ...
      if (canHoanTat != -1)
        moreActionDropdown.push({
          Type: "StatusID", Name: "Hoàn tất nhận hàng", Code: "check-outline", Link: "2", Actived: true, LstChild: []
        })
  ...
  openDetail(isAdd: boolean) {
    this.changeModuleData_sst = this.menuService.changeModuleData().subscribe((item: ModuleDataItem) => {
      ...
      if (Ps_UtilObjectService.hasValue(parent) && Ps_UtilObjectService.hasListValue(parent.LstChild)) {
        var detail = parent.LstChild.find(f => f.Code.includes('inport-list')
          || f.Link.includes('inport-list'))

        if (Ps_UtilObjectService.hasValue(detail) && Ps_UtilObjectService.hasListValue(detail.LstChild)) {
          var detail2 = detail.LstChild.find(f => f.Code.includes('inport-detail')
            || f.Link.includes('inport-detail'))
  ...
  resetFilter() {
    ...
    //prod
    this.filterFromWarehouseName.value = null
  ...
  search() {
    ...
    if (Ps_UtilObjectService.hasValueString(searchQuery)) {
      this.filterFromWarehouseName.value = searchQuery
