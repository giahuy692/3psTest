...
@Component({
  selector: 'app-ecom007-inport-detail',
  templateUrl: './ecom007-inport-detail.component.html',
  styleUrls: ['./ecom007-inport-detail.component.scss']
})
export class Ecom007InportDetailComponent implements OnInit, OnDestroy {
  loading = false
  isFilterActive = true
  excelValid = true
  ...
  UpdateInport(prop: string[] = [], items: DTOInport = this.inport) {
    ...
    if (this.isAdd && prop.findIndex(s => s == 'ToWarehouse')) {
      prop.push('ToWarehouse')
      this.inport.ToWarehouse = this.mainWarehouse
    }

    this.UpdateInportStatus_sst = this.apiService.UpdateInportReceive(items, prop).subscribe(res => {
      if (Ps_UtilObjectService.hasValue(res) && res.StatusCode == 0) {
        this.layoutService.onSuccess(`${ctx} thành công`)
        this.inport = res.ObjectReturn
      } else
  ...
  checkProp() {
    ...
    var wh = this.listWarehouse.find(s => s.Code == this.inport.FromWarehouse)
  ...
  onDropdownlistClick(ev, prop?: string) {
    if (Ps_UtilObjectService.hasValueString(prop) && ev != null) {
      this.warehouse = ev
      this.inport.FromWarehouse = this.warehouse.Code
      this.inport.FromWarehouseName = this.warehouse.WHName
      this.UpdateInport([prop])
    }
  }
  onSubmitSP(): void {
