...
import { LogAPIService } from '../../shared/services/log-api.service';
import { LayoutAPIService } from 'src/app/p-app/p-layout/services/layout-api.service';
import { DomSanitizer } from '@angular/platform-browser';

@Component({
  selector: 'app-log001-hachi24h-list',
  templateUrl: './log001-hachi24h-list.component.html',
  styleUrls: ['./log001-hachi24h-list.component.scss']
})
export class Log001Hachi24hListComponent implements OnInit, OnDestroy {
  loading = false
  isAdd = true
  isFilterActive = true
  ...
  filterIsHachi24: FilterDescriptor = {
    field: "IsHachi24", operator: "eq", value: 1
  }
  //search prod
  ...
  UpdateProductHachi24hByID_sst: Subscription
  DeleteProductHachi24hByID_sst: Subscription
  ...
  constructor(
    ...
    public apiService: LogAPIService,) { }
  ...
  loadFilter() {
    ...
    this.gridState.filter.filters = [this.filterIsHachi24]
  ...
  GetProductByBarcode(prod = this.curProd) {
    ...
    this.GetProductByBarcode_sst = this.apiService.GetProductByBarcode(prod.Barcode).subscribe(res => {
      if (Ps_UtilObjectService.hasValue(res) && Ps_UtilObjectService.hasValue(res.ObjectReturn) && res.StatusCode == 0) {
  ...
  UpdateProduct(prodID = [this.curProd.Code]) {
    ...
    this.UpdateProductHachi24hByID_sst = this.apiService.UpdateProductHachi24hByID(prodID).subscribe(res => {
  ...
  DeleteProduct(prodID = [this.curProd.Code]) {
    ...
    this.DeleteProductHachi24hByID_sst = this.apiService.DeleteProductHachi24hByID(prodID).subscribe(res => {
      if (Ps_UtilObjectService.hasValue(res) && res.StatusCode == 0) {
  ...
  exportExcel() {
    ...
    this.ExportExcel_sst = this.apiService.GetExcelAlbumn(3, 0).subscribe(res => {
      if (res != null) {
        Ps_UtilObjectService.getFile(res, getfileName)
        this.layoutService.onSuccess(`${ctx} thành công`)
      } else {
  ...
  p_ImportExcel(file) {
    ...
    this.ImportExcelAlbum_sst = this.apiService.ImportExcelAlbum(file, 3, 0).subscribe(res => {
      if (Ps_UtilObjectService.hasValue(res) && res.StatusCode == 0) {
        this.GetListProduct()
        this.layoutService.onSuccess(`${ctx} thành công`)
  ...
  ngOnDestroy(): void {
    ...
    this.UpdateProductHachi24hByID_sst?.unsubscribe()
    this.DeleteProductHachi24hByID_sst?.unsubscribe()
