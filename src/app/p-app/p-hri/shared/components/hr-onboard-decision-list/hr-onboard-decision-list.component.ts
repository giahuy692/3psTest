...
export class HrOnboardDecisionListComponent implements OnInit, OnDestroy {
  ...
  */
  APIGetListHRDecisionMaster() {
    ...
    // this.keySearch = "HVN3931435"
    this.decisionService.GetListHRDecisionMaster(this.gridState, this.keySearch, this.TypeDecision).pipe(takeUntil(this.unsubscribe)).subscribe((res: DTOResponse) => {
 ...
 */
  ...
  */
  handleRequiredFieldCheck(dto: DTOHRDecisionMaster, isSkipMsg: boolean = false) {
    let msgStr = `Đã xảy ra lỗi khi cập nhật trạng thái quyết định tuyển dụng ${dto.DecisionID ?? 'không xác định'}: Quyết định thiếu `;
