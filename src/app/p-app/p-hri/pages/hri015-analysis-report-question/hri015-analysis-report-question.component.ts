...
@Component({
  selector: 'app-hri015-analysis-report-question',
  templateUrl: './hri015-analysis-report-question.component.html',
  styleUrls: ['./hri015-analysis-report-question.component.scss']
})
export class Hri015AnalysisReportQuestionComponent implements OnInit {
  ...
  getQuizQuestionReport() {
    this.loading = true;
    let getListQuizReportAPI = this.quizSessionApiService.GetQuizQuestionReport(this.gridStateReport).subscribe(res => {
  ...
  onExportExcel() {
    this.loading = true
    var ctx = "Xuất Excel"
    var getfileName = "BaoCaoPhanTichCauHoi.xlsx"
    ...
    let onExportExcel =  this.quizSessionApiService.ExportQuestionAnalsisReport(filter).subscribe(res => {
     this.loading = false;
