...
export class Mar002DiscountComboComponent implements OnInit, OnDestroy {
  ...
  GetListPromotion_sst: Subscription
  GetWarehouse_stt: Subscription
  ...
  p_GetListPromotion() {
    ...
    this.GetListPromotion_sst = this.apiService.GetListPromotion(this.gridDSState, this.currentDonVi == null ? null : this.currentDonVi.value).subscribe(res => {
  ...
  p_GetDonViApDung() {
    ...
    this.GetWarehouse_stt = this.layoutApiService.GetWarehouse().subscribe((res: DTOWarehouse[]) => {
      if (res != null) {
  ...
  openPromotionDetail(isAdd: boolean) {
    this.changeModuleData_sst = this.menuService.changeModuleData().subscribe((item: ModuleDataItem) => {
      ...
      if (isAdd) {
        ...
        prom.TypeData = 2// this.currentTaoMoi.value
  ...
  ngOnDestroy(): void {
    ...
    this.GetListPromotion_sst?.unsubscribe()
    this.GetWarehouse_stt?.unsubscribe()
